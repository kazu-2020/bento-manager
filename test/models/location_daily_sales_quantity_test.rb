...
class LocationDailySalesQuantityTest < ActiveSupport::TestCase
  ...
  # 集計対象はこの販売先の販売のみ。他テストクラスの fixture が混ざらないよう、
  # 共有の販売先 fixture ではなくテストごとに専用の販売先を作る。
  setup do
  ...
  test "販売先の直近1ヶ月の日次販売個数が顧客区分ごとに集計される" do
    ...
    staff_sale = create_sale(location: @location, customer_type: :staff, sale_datetime: sale_date)
    create_sale_item(sale: staff_sale, quantity: 2)

    citizen_sale = create_sale(location: @location, customer_type: :citizen, sale_datetime: sale_date)
  ...
  test "取り消された販売は集計に含まれない" do
    ...
    voided_sale = create_sale(location: @location, customer_type: :staff, sale_datetime: sale_date,
    ...
    completed_sale = create_sale(location: @location, customer_type: :staff, sale_datetime: sale_date)
