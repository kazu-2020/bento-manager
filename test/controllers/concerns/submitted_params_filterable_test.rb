...
class SubmittedParamsFilterableTest < ActiveSupport::TestCase
  # concern はコントローラーの private メソッドとして submitted_params を足すため、
  # params を持つだけの最小の受け口を用意して実物の実装をそのまま呼ぶ
  class Receiver
    ...
    def filter(**shape)
      submitted_params(:cart, **shape)
    end
  end

  def filter(raw, **shape)
    Receiver.new(cart: raw).filter(**shape)
  end
  ...
  test "drops an undeclared collection key because it is not an id" do
    filtered = filter({ "coupon" => { "5" => { "quantity" => "1" } } })
  ...
  test "returns an empty hash when the key is missing or not a hash" do
    assert_equal({}, Receiver.new({}).filter)
