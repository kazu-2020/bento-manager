...
module SalesAnalyses
  class SummariesControllerTest < ActionDispatch::IntegrationTest
    ...
    test "認証済みユーザーが summary にアクセスできる" do
      get sales_analyses_summary_path(location_id: locations(:city_hall).id, days: 30)
    ...
    test "対応していない集計日数を指定すると過去30日として扱われる" do
      ...
      get sales_analyses_summary_path(location_id: location.id, days: 999)

      assert_select "p.text-staff", text: "2個"

      get sales_analyses_summary_path(location_id: location.id, days: 7)
