...
class StatusBadgeComponentPreview < ViewComponent::Preview
  # @label 有効
  def active
    render(StatusBadgeComponent.new(status: :active))
  end

  # @label 無効
  def inactive
    render(StatusBadgeComponent.new(status: :inactive))
  end
  ...
  def with_params(status: :active, model: :location)
    render(StatusBadgeComponent.new(status: status.to_sym, model: model.to_sym))
  end
end
