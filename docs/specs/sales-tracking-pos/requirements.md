...
3. The システム shall 追加発注可能な弁当を当日の日次在庫登録済み弁当に限定する
...
2. When クーポン枚数が入力される, the システム shall 購入した弁当の合計個数（複数種類の弁当がある場合は各 quantity の合計）を超えるクーポン枚数を受け付けない
...
8. The システム shall 「購入した弁当1個につき最大1枚（弁当の種類ではなく、個数ベースでカウント）」の適用条件をクーポンマスタで管理する
9. If 弁当数量が0でクーポン枚数が1以上入力される, then the システム shall エラーメッセージを表示し販売を拒否する

### Requirement 13: サイドメニュー（サラダ）の条件付き価格設定
...
### Requirement 14: 返品・返金処理（取消・再販売・差額返金）
...
### Requirement 16: 価格ルール適用時の価格存在検証
...
### Requirement 17: 管理画面での価格設定不備の警告表示
...
- 対応する CatalogPrice が存在しない場合、販売時にエラーが発生する（Requirement 16）
...
### Requirement 18: 価格ルール作成・有効化時の価格存在バリデーション
