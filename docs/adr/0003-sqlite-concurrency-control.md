...
対象の行がまだ存在しない種類の一意性は、アプリ側のガードではなく一意インデックスで守る。例は `catalog_discontinuations.catalog_id` の一意インデックス（`db/schema.rb`）で、提供終了が二重に記録されない保証はここが持っている。`refunds.original_sale_id` も同じ形で、1 つの販売が二度は差額精算されない保証をここが持つ（`Sale#void!` は既に読まれた行に対しては窓が開くため、決定 1 の直列化とは別に必要）。
