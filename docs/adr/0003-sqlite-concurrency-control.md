...
対象の行がまだ存在しない種類の一意性は、アプリ側のガードではなく一意インデックスで守る。例は `catalog_discontinuations.catalog_id` の一意インデックス（`db/schema.rb`）で、提供終了が二重に記録されない保証はここが持っている。`refunds` の `original_sale_id` / `corrected_sale_id` も同じ形で、1 つの販売が二度は差額精算されない保証と、1 つの販売が二度は修正後の販売にならない保証をここが持つ。前者は `Sale#void!` が既に読まれた行に対しては窓を開けるため決定 1 の直列化とは別に必要で、後者にはアプリ側のガードがそもそも無い。
