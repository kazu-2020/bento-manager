...
- **退役したトークンは frontmatter から消す。**CSS 側には別名として残す（削ると変数が未定義になり `bg-accent` / `text-accent` が無効値になるため。`bento` は daisyUI の組み込みテーマではなく、既定値のマージも `themes.css` の出力も起きない）が、frontmatter に残すと機械的に読むエージェントが選択可能なトークンとして扱ってしまう。
