...
- **継続中の失敗は再通知されない。** `trigger_conditions` はいずれも状態遷移（新規作成・解決からの再発）でしか発火しないため、失敗が 3 日続いても届くのは初日の 1 通だけになる。2 日目以降の沈黙は正常時と区別できず、初日のメールを取りこぼすと無期限に無音になる。ADR-0001 決定 1 が最大のリスクとした「失敗していたことに誰も気づかない」状態を、通知の側で再現しうる。定期リマインドには `legacy_trigger_conditions` が要るが、プロバイダが将来の deprecation を明記しているため採否は保留した（#282）
