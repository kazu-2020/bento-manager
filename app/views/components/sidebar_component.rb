...
class SidebarComponent < ApplicationComponent
  MenuItem = Data.define(:path, :label, :icon, :path_prefix)
  ...
  def menu_items
    @menu_items ||= [
      ...
      MenuItem.new(
        path: helpers.admin_employees_path,
        label: "従業員",
        icon: :users,
        path_prefix: "/admin/employees"
      ),
      MenuItem.new(
        path: helpers.locations_path,
        label: "配達場所",
        icon: :location,
        path_prefix: "/locations"
      ),
      MenuItem.new(
        path: helpers.catalogs_path,
        label: "カタログ",
        icon: :catalog,
        path_prefix: "/catalogs"
      )
    ]
  end
  ...
  def menu_item_class(item)
    base_class = "flex items-center gap-3"
    if active?(item)
      "#{base_class} active bg-primary/10 text-primary font-medium"
    else
      "#{base_class} hover:bg-base-300"
    end
  end
